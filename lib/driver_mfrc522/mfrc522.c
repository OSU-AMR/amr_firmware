 ...
 */
MFRC522Ptr_t MFRC522_Init(uint cs_pin) {
...
void PCD_Init(MFRC522Ptr_t mfrc, PIO pio_inst, uint miso_pin, uint mosi_pin, uint sck_pin, uint cs_pin) {
    ...
    PCD_WriteRegister(mfrc, TPrescalerReg,
                      0xA9);  // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie
                              ...
                              // of 25�s.
    PCD_WriteRegister(mfrc, TReloadRegH,
...
StatusCode PCD_TransceiveData(MFRC522Ptr_t mfrc,
    ...
    return PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, sendData, sendLen, backData, backLen, validBits,
                                   rxAlign, checkCRC);
...
StatusCode PCD_CommunicateWithPICC(MFRC522Ptr_t mfrc,
    ...
    if (command == PCD_Transceive) {
        PCD_SetRegisterBitMask(mfrc, BitFramingReg,
    ...
    uint8_t errorRegValue = PCD_ReadRegister(mfrc, ErrorReg);  // ErrorReg[7..0] bits are: WrErr
    ...
    if (backData && backLen) {
        ...
        *backLen = n;  // Number of uint8_ts returned
        ...
        _validBits = PCD_ReadRegister(mfrc, ControlReg) & 0x07;  // RxLastBits[2:0] indicates the number of valid bits
...
StatusCode PICC_Select(MFRC522Ptr_t mfrc,
    ...
    while (!uidComplete) {
        ...
        uint8_t uint8_tsToCopy =
            currentLevelKnownBits / 8 + (currentLevelKnownBits % 8 ? 1 : 0);  // The number of uint8_ts
        ...
        while (!selectDone) {
            ...
            result = PCD_TransceiveData(mfrc, buffer, bufferUsed, responseBuffer, &responseLength, &txLastBits, rxAlign,
                                        false);
            if (result == STATUS_COLLISION) {                              // More than one PICC in the field
                                                                           // => collision.
                uint8_t valueOfCollReg = PCD_ReadRegister(mfrc, CollReg);  // CollReg[7..0] bits are: ValuesAfterColl
...
StatusCode MIFARE_Write(MFRC522Ptr_t mfrc,
    ...
    result = PCD_MIFARE_Transceive(mfrc, cmdBuffer, 2,
    ...
    result = PCD_MIFARE_Transceive(mfrc, buffer, bufferSize,
                                   false);  // Adds CRC_A and checks that the response is MF_ACK.
    if (result != STATUS_OK) {
        return result;
    }
...
StatusCode MIFARE_Ultralight_Write(MFRC522Ptr_t mfrc,
    ...
    // Perform the write
    result = PCD_MIFARE_Transceive(mfrc, cmdBuffer, 6,
                                   false);  // Adds CRC_A and checks that the response is MF_ACK.
    if (result != STATUS_OK) {
        return result;
    }
...
StatusCode MIFARE_TwoStepHelper(MFRC522Ptr_t mfrc,
    ...
    // Step 2: Transfer the data
    result = PCD_MIFARE_Transceive(mfrc, (uint8_t *) &data, 4,
                            ...
                            uint8_t pACK[])  // Authenticate with 32bit password
{
    ...
    uint8_t validBits = 0;
    uint8_t rxlength = 5;
    result = PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, cmdBuffer, 7, cmdBuffer, &rxlength, &validBits, 0,
                                     false);
...
StatusCode PCD_MIFARE_Transceive(MFRC522Ptr_t mfrc,
    ...
    uint8_t cmdBufferSize = sizeof(cmdBuffer);
    uint8_t validBits = 0;
    result = PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, cmdBuffer, sendLen, cmdBuffer, &cmdBufferSize,
                                     &validBits, 0, false);
...
void PCD_DumpVersionToSerial(MFRC522Ptr_t mfrc) {
    ...
    LOG_INFO("RFID Firmware Version 0x%s", string);
    // Lookup which version
    switch (v) {
    case 0x88:
        LOG_INFO("Version = (clone)");
        break;
    case 0x90:
        LOG_INFO("Version = v0.0");
        break;
    case 0x91:
        LOG_INFO("Version = v1.0");
        break;
    case 0x92:
    ...
    if ((v == 0x00) || (v == 0xFF))
        LOG_ERROR("WARNING: Communication failure, is the MFRC522 properly "
                  "connected?");
...
void PICC_DumpToSerial(MFRC522Ptr_t mfrc, Uid *uid  ///< Pointer to Uid struct
    ...
    case PICC_TYPE_MIFARE_PLUS:
    case PICC_TYPE_TNP3XXX:
        LOG_INFO("Dumping memory contents not implemented for that PICC type.");
        break;
...
void PICC_DumpDetailsToSerial(Uid *uid  ///< Pointer to Uid struct returned from
    ...
    PICC_Type piccType = PICC_GetType(uid->sak);
    LOG_INFO(" PICC type: ");
    LOG_INFO(PICC_GetTypeName(piccType));
 ...
 */
void MIFARE_SetAccessBits(uint8_t *accessBitBuffer,  ///< Pointer to uint8_t 6, 7 and 8 in the sector
...
bool MIFARE_OpenUidBackdoor(MFRC522Ptr_t mfrc, bool logErrors) {
    ...
    uint8_t received;
    StatusCode status =
        PCD_TransceiveData(mfrc, &cmd, (uint8_t) 1, response, &received, &validBits, (uint8_t) 0, false);  // 40
    if (status != STATUS_OK) {
        if (logErrors) {
            LOG_ERROR("Card did not respond to 0x40 after HALT command. Are you "
                      "sure it is a UID changeable one?\r\n");
            LOG_ERROR("Error name: ");
            LOG_ERROR(GetStatusCodeName(status));
        }
        return false;
    }
    if (received != 1 || response[0] != 0x0A) {
        if (logErrors) {
            LOG_ERROR("Got bad response on backdoor 0x40 command: ");
    ...
    if (status != STATUS_OK) {
        if (logErrors) {
            LOG_ERROR("Error in communication at command 0x43, after "
    ...
    if (received != 1 || response[0] != 0x0A) {
        if (logErrors) {
            LOG_ERROR("Got bad response on backdoor 0x43 command: ");
...
bool MIFARE_SetUid(MFRC522Ptr_t mfrc, uint8_t *newUid, uint8_t uidSize, bool logErrors) {
    ...
    if (!newUid || !uidSize || uidSize > 15) {
        if (logErrors) {
            LOG_ERROR("New UID buffer empty, size 0, or size > 15 given\r\n");
        }
        return false;
    }
    ...
    if (status != STATUS_OK) {
        if (status == STATUS_TIMEOUT) {
            ...
            if (!PICC_IsNewCardPresent(mfrc) || !PICC_ReadCardSerial(mfrc)) {
                LOG_ERROR("No card was previously selected, and none are "
            ...
            if (status != STATUS_OK) {
                ...
                if (logErrors) {
                    LOG_ERROR("Failed to authenticate to card for reading, "
                              "could not set UID: \r\n");
                    LOG_ERROR(GetStatusCodeName(status));
                }
                return false;
            ...
            if (logErrors) {
                LOG_ERROR("PCD_Authenticate() failed: ");
                LOG_ERROR(GetStatusCodeName(status));
                LOG_ERROR("\n");
            }
            return false;
    ...
    if (status != STATUS_OK) {
        if (logErrors) {
            LOG_ERROR("MIFARE_Read() failed: ");
            LOG_ERROR(GetStatusCodeName(status));
            LOG_ERROR("Are you sure your KEY A for sector 0 is 0xFFFFFFFFFFFF?\r\n");
        }
        return false;
    }
    ...
    if (!MIFARE_OpenUidBackdoor(mfrc, logErrors)) {
        if (logErrors) {
            LOG_ERROR("Activating the UID backdoor failed.\r\n");
        }
        return false;
    }
